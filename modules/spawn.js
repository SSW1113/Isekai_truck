...
export function createSpawnSystem(monsterSystem) {
    ...
    function getSpawnPosition(truck) {
        ...
        const distance =
            ...
            Math.random() *
            (SPAWN_CONFIG.maxDistance - SPAWN_CONFIG.minDistance);
    ...
    // 몬스터 한 마리 생성
    function spawn(truck) {
        const typeId = chooseMonsterType();
        const position = getSpawnPosition(truck);

        monsterSystem.createMonster(typeId, position.x, position.z);
    }
    ...
    function update(truck) {
        ...
        const needCount =
            SPAWN_CONFIG.targetCount - currentCount;

        const spawnCount =
            Math.min(needCount, SPAWN_CONFIG.spawnPerInterval);
