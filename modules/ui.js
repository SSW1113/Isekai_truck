export function createPlayerHUD() {
    ...
    function update(state) {
        ...
        pointText.textContent = `포인트 ${state.upgradePoints}`;
...
export function createUpgradeUI(player, truck, onChange) {
    ...
    function update() {
        ...
        pointText.textContent = `남은 포인트: ${playerState.upgradePoints}`;

        speedLevel.textContent = `Lv.${truckStats.speedLevel}`;
        sizeLevel.textContent = `Lv.${truckStats.sizeLevel}`;

        speedStat.textContent = `최대 속도: ${truckStats.maxSpeed.toFixed(3)}`;
        sizeStat.textContent = `트럭 크기: ${Math.round(truckStats.sizeScale * 100)}%`;
